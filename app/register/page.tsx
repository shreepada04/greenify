...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.email || !formData.password) {
      ...
      warning('Missing Information', 'Please fill in all fields')
      setLoading(false)
      return
    }
    ...
    if (formData.password.length < 6) {
      ...
      warning('Weak Password', 'Password must be at least 6 characters long')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      if (registerSuccess) {
        success('Account Created Successfully! 🎉', 'Welcome to Greenify! Redirecting to dashboard...')
      ...
      showError('Registration Error', 'An error occurred. Please try again.')
    } finally {
      setLoading(false)
    }
  }
